...
func main() {
	...
	// Data sending loop for Microservice 2
	go func() {
		for i := 0; i < 10; i++ {
			data := []byte("Message from Microservice 2: " + string(i))
	...
	// Data receiving loop for Microservice 2
	...
	for {
		...
		case msg := <-partitionConsumer.Messages():
			log.Printf("Microservice 2 Received message: %s\n", string(msg.Value))
		}
	}
}
